---
title: "Requirements"
sidebar_position: 8
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

:::tip Concept Overview
Requirements are not just statements of need — they are **alignment tools**, **boundary‑setting tools**, and **decision‑making tools**.

For a Technical Analyst, requirements are how you:
- Make implicit assumptions explicit
- Anchor conversations in facts
- Create shared accountability
- Reduce ambiguity and future conflict
:::

---

# 🧭 The TA Mental Model for Requirements

A requirement is **not** a wish list item.
It is a **constraint**, **condition**, or **decision rule** that must be met for a solution to be viable.

**Backbone of the TA Role: Context → Problem → Constraints → Requirements → Acceptance → Decision**

---

## 🧩 What Good Requirements Look Like

A good requirement is:

- [x] **Clear** — no ambiguity
- [x] *Testable** — you can verify if it’s met
- [x] **Bounded** — it has a scope
- [x] **Neutral** — no blame, no emotion
- [x] **Contextualised** — tied to a real constraint
- [x] **Negotiable or Non‑Negotiable** — explicitly stated
- [x] **Traceable** — linked to a problem or risk

---

##  🧱 The TA Requirement Pattern

Every requirement you write should follow this structure:

1. **Context**
> - Why this requirement exists.

2. **Constraint**
> - What limits the solution space.

3. **Requirement Statement**
> - A single sentence, structured as:

```
The solution must <do/enable/support> <behaviour or condition> because <reason>.
```

4. **Acceptance Criteria**
> - How we know the requirement is met.

5. **Negotiability**
> - **Non‑negotiable** (fixed constraint)
> - **Negotiable** (preference)
> - **Team‑specific** (must be supported via extension points)

---

# 🧩 Example Requirements (Side‑by‑Side)

<Tabs>

<TabItem value="good" label="Good Requirement">

```
Context:
The team deploys Python services to Kubernetes using a shared base image.

Constraint:
The base image is maintained by the platform team and cannot be modified.

Requirement:
The cookiecutter template must support using the shared Python base image because teams cannot maintain custom images.

Acceptance Criteria:
- Dockerfile references the shared base image
- No custom OS-level dependencies required
- Build pipeline succeeds using the shared image

Negotiability:
Non‑negotiable
```

</TabItem>

<TabItem value="bad" label="Bad Requirement">

```
We should use the platform image because that’s what everyone else does.
```

</TabItem>

</Tabs>

---

## 🧩 Requirements Presentation Structure

1. **Executive Summary**
- What you analysed: _I analysed the current workflow, tooling, and constraints across the team._
- What you found: _I found inconsistencies in environments, duplicated effort in setup, and unclear ownership around several technical decisions._
- What decisions are needed: _Decisions are needed on which components are fixed, which can vary by team, and which require additional support before adoption._

2. **Project Context**
- Team: # engineers across backend, data, and QA; mixed experience levels.
- Tech stack: Python, Node.js, Docker, GitHub Actions, AWS.
- Constraints: Security baselines, shared infrastructure, limited migration window, dependency on central platform team.
- Deployment model: Containerised services deployed via CI/CD to AWS ECS.

3. **Fixed vs Flexible vs Team‑Specific**
A table like this:

| Category | Items | Notes |
|---------|-------|-------|
| Fixed | Python 3.12, shared base image | Cannot change |
| Flexible | Test framework | Can change with effort |
| Team‑specific | ETL folder structure | Must be supported |

4. **Requirements List**
Each requirement in the TA pattern format.
- Trigger:
- Action:
- Outcome:

5. **Adoption Conditions**
What must be true for the team to adopt the template.
- Documentation is available for setup, testing, and deployment.
- Teams can migrate incrementally without blocking current work.
- Support is available for onboarding and troubleshooting during the transition.
- Tooling is compatible with existing security and compliance requirements.

6. **Risks & Dependencies**
What could block adoption.
- Dependency on platform team for base image updates.
- Risk of adoption delays if documentation is incomplete.
- Potential incompatibility with legacy services.
- Need for alignment with security and DevOps teams.

7. **Next Steps**
Clear, shared actions — not all assigned to you.
- Finalise the fixed/flexible/team‑specific decisions with engineering leads.
- Validate requirements with each team to confirm feasibility.
- Produce the first draft of the template and run a pilot with one team.
- Gather feedback and iterate before wider rollout.

---

## 🛡️ BAU

1. Operate from written requirements
> - Never rely on verbal agreements.

2. Use the TA requirement pattern to create clarity
> - It reduces political manoeuvring.

3. Document alignment decisions
> - This protects you from blame shifting.

4. Set boundaries early
> - “This is what we agreed. This is what I’m delivering.”

5. Avoid taking on unbounded work
> - If it’s not in the requirements, it’s not your responsibility.

6. Protect your energy
> _ Ensure you have stability to operate as you need

