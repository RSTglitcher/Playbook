...
Container orchestration management deploys, manages, and scales containerised applications. Processes such as
deployment, scaling, load balancing and high availability are automated.
...
It makes the overall CI/CD process more efficient and scalable as it can be integrated with CI/CD to automate the software delivery
pipeline from code commit to application deployment.

| CICD Feature                     | Purpose                                                                                                                                                                                                                                                               |
|----------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Containerization                 | Kubernetes works well with containerized applications. As part of the CI/CD process, developers often containerize their applications using technologies like Docker. Kubernetes can then orchestrate the deployment and scaling of these containerized applications. |
| Deployment Orchestration         | In the CI/CD pipeline, after code has been built, tested, and packaged into containers, Kubernetes can be used to orchestrate the deployment of these containers to different environments, such as staging or production.                                            |
| Scaling and Load Balancing:      | Kubernetes provides features for scaling applications horizontally by adding or removing instances of containers based on demand. This capability is valuable in ensuring that applications can handle varying workloads efficiently.                                 |
| Rolling Updates and Rollbacks    | Kubernetes supports rolling updates, enabling organizations to deploy new versions of their applications gradually, minimizing downtime. If an issue arises during deployment, Kubernetes supports easy rollbacks to the previous version.                            |
| Configuration Management         | Kubernetes uses configuration files (YAML manifests) to define the desired state of applications, including deployment configurations, services, and other resources. These configuration files can be versioned, making them a part of the CI/CD pipeline.           |
| Integration with CI/CD Tools     | CI/CD tools often integrate with Kubernetes to automate the deployment process. CI/CD pipelines can trigger deployments to Kubernetes clusters, allowing for seamless integration between the development and deployment phases.                                      |
| Service Discovery and Networking | Kubernetes provides built-in service discovery and networking capabilities, allowing applications to communicate with each other within the cluster. This facilitates the development and deployment of microservices-based architectures.                            |

</TabItem>
<TabItem value="kubectl" label="Kubectl">

The Kubernetes CLI tool, [kubectl](https://kubernetes.io/docs/reference/kubectl/) communicates with Kubernetes clusters via the Kubernetes API.

Otherwise referred to as k8s (# of letters between the K and S in Kubernetes)
