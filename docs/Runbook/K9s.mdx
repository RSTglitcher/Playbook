...
│ ├── Validate workloads & storage
│ ├── Inspect logs & events
│ └── Observe autoscaling & network
└── SWE Skills
├── Deploy/Manage workloads
├── Debug & inspect pods
├── Scale & rollout workloads
...
**How to formulate requirements when working with K9s**

* How to formulate requirements when working with K9s
* Determine clusters and contexts to monitor
* Identify which namespaces and workloads need visibility
* Define what logs, metrics, and events are required for monitoring
* Confirm which RBAC permissions are needed for TA access
* Establish workflows for investigating incidents and performing triage
...
<Tabs>
<TabItem value="skill" label="Skills">
...
* Determine which workloads, namespaces, and clusters require management via K9s
* Document workflows for deployment, debugging, and scaling
* Define alerting or monitoring touchpoints to integrate K9s usage
* Provide TA handover: access levels, namespaces, and intended workflows for troubleshooting
* Establish best practices for using K9s with CI/CD and cluster operations
